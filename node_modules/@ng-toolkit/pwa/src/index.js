 ...
 */
function checkAngularPWA(options) {
    return (tree) => {
        const cliConfig = JSON.parse(test_1.getFileContent(tree, `${options.directory}/angular.json`));
        const project = cliConfig.projects[options.clientProject];
...
function addEntryServerModule(options) {
    return (tree) => {
...
function updateSWMechanism(options) {
    return (tree, context) => {
        ...
        if (!ngToolkitSettings.pwa) {
            ...
            if (methodBodyEdges) {
                fileContent = fileContent.substring(0, methodBodyEdges.start) + outdent_1.default({ trimLeadingNewline: false }) `
